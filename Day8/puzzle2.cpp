...
bool check_circuits_map(map<int, vector<int>>& circuit_to_coordinates, size_t total_coordinates_count){
    //cout<<total_coordinates_count<<endl;
    if(circuit_to_coordinates.size()!=1){
        return true;
    }
    else{
        auto it = circuit_to_coordinates.begin();
        vector<int> coordinates = it->second;
        if(coordinates.size()!=total_coordinates_count){
            return true;
        }
        return false;
    }
...
int main() {
    ...
    int lastx1=0;
    int lastx2=0;
    while(check_circuits_map(circuit_to_coordinates, coordinates.size()) || circuit_to_coordinates.size()==0){
        DistancePair curr_pair = distance_q.top();
        int first_pos = curr_pair.coordinate1_pos;
        int second_pos = curr_pair.coordinate2_pos;
        lastx1 = coordinates[first_pos][0];
        lastx2 = coordinates[second_pos][0];
